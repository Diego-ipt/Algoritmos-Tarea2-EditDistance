...
inline int editDistanceRecursive(const std::string &s, const std::string &t) {
...
inline int editDistanceMemo(const std::string &s, const std::string &t) {
    int lcs = lcs_memo_str(s, t);
    return static_cast<int>(s.size() + t.size() - 2 * lcs);
}

// Calcula la distancia de edición usando programación dinámica (tabla 2D completa).
inline int editDistanceDP(const std::string &s, const std::string &t) {
...
inline int editDistanceDPOptimized(const std::string &s, const std::string &t) {
    int n = static_cast<int>(s.size()), m = static_cast<int>(t.size());
